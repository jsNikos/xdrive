...
/**
 * These connection-requests are checked to have logged-in session. In case they
 * are accepted and will get added to connections and adding a destroy-listener
 * to the session which triggers to remove the connection.
 */
addAuthenticatedConnection(request){
 // TODO check for logged-in session
 // add session-destroy listener to session
 var connection = this.addConnection(request);
 // var orig_destroy = session.destroy; !! this doesn't work because method
	// not deserialized
